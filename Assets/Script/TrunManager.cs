...
public class TrunManager : MonoBehaviour
{
    ...
    {
        Transform targetpos = P1_pos.transform.position + steps;
        if ((pawn[.transform.position = Grids[targetPos % totalGrids].transform.position;) || (Pawn[1].transform.position == Grids[currentGrid].transform.position))
        {
            Ismoving = false;
        }
